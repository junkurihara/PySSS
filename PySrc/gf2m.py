...
class gf2m:
    ...
    def setDegree(self, size:int) -> None:
        if size > gf2m.MAXDEGREE:
            ...
            self.mul = self.__genMul(self.mord, self.poly)
            self.idx = self.__genIdx(self.mord, self.mul)


    # private functions
    def __genMul(self, mord: int, poly: int) -> Sequence[T]:
    ...
    def __genIdx(self, mord: int, mul: Sequence[T]) -> Sequence[T]:
        #print("{0}: Generate index table idx[] that maps the value to the exponent (e.g., when mul[i] = j, idx[j] = i)".format(self))
    ...
    def mulGF2m(self, x: int, y: int) -> int:
        try:
    ...
    def divGF2m(self, x: int, y: int) -> int:
        try:
...
if __name__ == '__main__':
    ...
    print("nvec1 + nvec2 = {0}".format(g.vaddGF2m(nvec1,nvec2)))
    print("nvec1 * nvec2 = {0}".format(g.vmulGF2m(nvec1,nvec2)))
    print("nvec1 / nvec2 = {0}".format(g.vdivGF2m(nvec1,nvec2)))
    print("nvec1^-1 = {0}".format(g.vinvGF2m(nvec1)))
